...
export default function ContactProfile() {
  ...
  const aiSummaryMutation = trpc.contacts.generateAiSummary.useMutation({
    onSuccess: () => { toast.success("AI summary generated"); utils.contacts.getProfile.invalidate({ id: Number(id) }); },
  ...
  const toggleStarMutation = trpc.contacts.toggleStar.useMutation({
    onSuccess: () => { utils.contacts.getProfile.invalidate({ id: Number(id) }); utils.contacts.list.invalidate(); },
  });
  const addNoteMutation = trpc.contacts.addNote.useMutation({
    onSuccess: () => { toast.success("Note added"); utils.contacts.getNotes.invalidate({ contactId: Number(id) }); setNewNote(""); },
    onError: () => toast.error("Failed to add note"),
  });
  const deleteNoteMutation = trpc.contacts.deleteNote.useMutation({
    onSuccess: () => { toast.success("Note deleted"); utils.contacts.getNotes.invalidate({ contactId: Number(id) }); },
  });
  ...
  const deleteDocMutation = trpc.contacts.deleteDocument.useMutation({
    onSuccess: () => { toast.success("Document deleted"); utils.contacts.getDocuments.invalidate({ contactId: Number(id) }); },
  });
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      ...
      uploadDocMutation.mutate({
        contactId: Number(id), title: docTitle || file.name,
  ...
  const startEditing = () => {
    ...
    setEditData({
      ...
      notes: profile.notes || "", category: profile.category || "other",
      riskTier: (profile as any).riskTier || "",
      complianceStage: (profile as any).complianceStage || "",
      influenceWeight: (profile as any).influenceWeight ?? "",
      introducerSource: (profile as any).introducerSource || "",
      referralChain: (profile as any).referralChain || "",
    });
    setEditing(true);
  };
